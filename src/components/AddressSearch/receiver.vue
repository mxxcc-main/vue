...
<template>
  <div class="home-container">
    ...
    <div v-if="!phoneSize" class="location-container" style="justify-content: space-between;">
      ...
      <!--<div class="content-title">{{$t('common.balance')}}</div>-->
      <!--<div class="content-detail"><span style="display: inline-block;max-width: 200px;text-overflow: ellipsis;overflow: hidden;">{{balance}}</span> {{tokenName}}</div>-->
      <!--</div>-->
      <!--</div>-->
      <!--<div class="detail-item-container">-->
      <!--<img src="../../../static/images/short_background@3x.png" class="short-background">-->
      <!--<div class="content-container">-->
      <!--<div class="content-title">{{tokenName}} {{$t('common.value')}}</div>-->
      ...
      <!--<img src="../../../static/images/short_background@3x.png" class="short-background">-->
      <!--<div class="content-container">-->
      <!--<div class="content-title">{{$t("home.statistics.transactionNumber")}}</div>-->
      <!--<div class="content-detail">{{tradeNum}} {{$t("home.statistics.transactionNumberUnit")}}</div>-->
      <!--</div>-->
      <!--</div>-->
      <div class="centerTop">
        <div>
          <div><span>{{$t('common.Tokenname')}}:</span> <span>{{tokenAll.name}}</span> </div>
          <div><span>{{$t('common.tokenSymbol')}}:</span>  <span>{{tokenAll.symbol}}</span> </div>
        </div>
        <div>
          <div><span>{{$t('common.Totalcirculation')}}:</span><span>{{tokenAll.totalSupply}}</span></div>
          <div><span>{{$t('common.Decimaldigit')}}:</span><span>{{tokenAll.decimals}}</span></div>
        </div>
        <div>
          <div><span>{{$t('common.Creatoraddress')}}:</span>  <span>{{tokenAll.creator}}</span></div>
          <div><span>{{$t('common.labelHash')}}:</span>  <span>{{tokenAll.hash}}</span></div>
        </div>
      </div>
    </div>
    ...
    <div v-if="!phoneSize" ref="addRessList">
      <!--<div style="width:calc(100vw * (621/1920));height: 50px; border-radius: 20px; margin: 30px auto;   display: flex;background:rgba(251,252,253,1);box-shadow:0px 2px 10px 1px rgba(0,160,233,0.16);justify-content: space-between">-->
        ...
        <!--<div   @click="transactionCCM()" class="buttonRadius" :class="isactive ? ' ':'active'"><span>CCM{{$t('common.TokenTrading')}}</span></div>-->
      <!--</div>-->
        ...
        <el-table :data="records"
          ...
          <el-table-column :label="$t('home.headerMenu.send')" >
            <template slot-scope="scope">
              ...
              <span style="color:#1EA8FF;cursor: pointer;max-width: calc(100vw * (130 / 1920));overflow: hidden;text-overflow: ellipsis;white-space: nowrap;display: inline-block;"@click="onClickAddress(scope.row.from)" >{{ scope.row.from || ''}}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column :label="$t('home.headerMenu.receiver')":width="200">
            <template slot-scope="scope">
              <!--<div style="display: flex;align-items: center;">-->
                <!--<img src="../../../static/images/icon_Into@3x.png" v-if="scope.row.to == id" class="trade-icon">-->
                <!--<img src="../../../static/images/icon_out@3x.png" v-else class="trade-icon">-->
                <!--<span style="max-width: calc(100vw * (130 / 1920));overflow: hidden;text-overflow: ellipsis;white-space: nowrap;color: #1EA8FF;display: inline-block;margin-left: 10px;cursor:pointer;" @click="onClickAddress(scope.row.realAddress)">{{ scope.row.realAddress || ''}}</span>-->
              <!--</div>-->
              ...
              <div  v-if="scope.row.to == id"  style="display: flex;align-items: center;">
                ...
                <span style="max-width: calc(100vw * (130 / 1920));overflow: hidden;text-overflow: ellipsis;white-space: nowrap;color: #1ea8ff;display: inline-block;margin-left: 10px;cursor: pointer"@click="onClickReceive(scope.row.realAddress)">{{ scope.row.realAddress || ''}}</span>
              </div>
    ...
    <div v-if="phoneSize" class="address-detail-container">
      ...
      <div class="address-detail-top">
        ...
        <div class="top2">
          <div class="centerTopPhone">
            <div  class="tokenAll" style="text-align: left">
              <div><span>{{$t('common.Tokenname')}}:</span> <span>{{tokenAll.name}}</span> </div>
              <div><span>{{$t('common.tokenSymbol')}}:</span>  <span>{{tokenAll.symbol}}</span> </div>
              <div><span>{{$t('common.Totalcirculation')}}:</span><span>{{tokenAll.totalSupply}}</span></div>
              <div><span>{{$t('common.Decimaldigit')}}:</span><span>{{tokenAll.decimals}}</span></div>
    ...
    <div v-if="phoneSize" class="phone-list-container " ref="iphoneList">
      <div    class="list-item-container" v-for="(item, index) in records" >
        <div class="list-item-top">
          ...
          <div v-if="item.to == id" class="trade-type-icon" style="background-color: #20BA05;">{{$t('common.enter')}}</div>
          <div v-else class="trade-type-icon" style="background-color: #E58335;">{{$t('common.Out')}}</div>
        </div>
  ...
  export default {
    ...
    methods:{
      ...
      tokenDetail(){
        ...
        let url = `${BASE_URL}/block/browser/tokenDetail`;
        let params={
...
<style scoped>
  ...
  .tokenAll>div>span:nth-of-type(1){
    font-size: 13px;
    /*font-weight: bold;*/
  ...
  .centerTop>div>div:last-of-type{
    border-top: 2px solid #1ea8ff;
    padding-top: 20px;
  }
  .centerTop>div>div{
    margin-top: 22px;
    overflow:hidden;text-overflow:ellipsis;white-space:nowrap;
  } .centerTop>div>div>span{
    font-size: 15px;
    margin-right: 5px;
    /*font-weight: bolder;*/
    color: #666666;
