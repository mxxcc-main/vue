  ...
  <div style="width: 100%;height: 100%;position: relative;overflow: hidden;">
    ...
    <div class="chart-model-container">
      <div class="chart-date-model">
        <div @click="onClickChangeDateMode(3)" :style="dateType == 3 ? 'color: #333333;' : ''">{{$t('common.year')}}</div>
        <div @click="onClickChangeDateMode(2)" :style="dateType == 2 ? 'color: #333333;' : ''">{{$t('common.week')}}</div>
        <div @click="onClickChangeDateMode(1)" :style="dateType == 1 ? 'color: #333333;' : ''">{{$t('common.day')}}</div>
      </div>
      <!--<div class="chart-date-model" style="width: 170px;">-->
        <!--<div style="border-radius: 20px;height: 24px;width: 70px;line-height: 24px;text-align: center;background-color: #1D233B;color: white;font-size: 12px;" @click="onClickChangeHashMode(1)"-->
             <!--:style="hashMode == 1 ? 'color: white;background-color:#1D233B;' : 'color: #1D233B;background-color:#56618D;'">哈希率</div>-->
        <!--<div  style="border-radius: 20px;height: 24px;width: 80px;line-height: 24px;text-align: center;background-color:  #56618D;color: white;font-size: 12px;color: #1D233B;" @click="onClickChangeHashMode(2)"-->
...
<script>
  ...
  import {getWeekDate, getYearWeek} from '@/common/js/utils'
      ...
      getChartStatistics() {
        ...
        this.$jsonp(url, params).then(data=>{
          ...
          if(this.dateType == 1) {
            ...
            date.forEach(ele=>{
              let arr = ele.split('-');
              this.xRange.push(`${Number(arr[1])}月${Number(arr[2])}日`);
            })
